...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ 
  ...
  return (
    <div className="py-32 bg-cyber-navy min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        
        {/* Header */}
        <div className="flex flex-col md:flex-row-reverse justify-between items-center mb-12 gap-6 bg-cyber-ice/5 p-8 rounded-3xl border border-cyber-powder/20 backdrop-blur-xl">
          <div className="text-right">
            <h1 className="text-3xl font-black text-white italic">مركز التحكم <span className="text-cyber-sapphire">الاستراتيجي</span></h1>
          ...
          <div className="flex gap-4">
            <button 
              ...
              className={`px-6 py-3 rounded-xl font-black text-xs uppercase tracking-widest transition-all ${activeTab === 'articles' ? 'bg-cyber-sapphire text-white' : 'bg-cyber-ice/10 text-slate-500'}`}
            >
              الأخبار والمقالات
            </button>
            <button 
              onClick={() => { setActiveTab('stories'); setShowAddForm(false); }}
              className={`px-6 py-3 rounded-xl font-black text-xs uppercase tracking-widest transition-all ${activeTab === 'stories' ? 'bg-cyber-sapphire text-white' : 'bg-cyber-ice/10 text-slate-500'}`}
            >
              القصص والتحقيقات
            </button>
          </div>
        ...
        {/* Add Form */}
        {showAddForm && (
          <form onSubmit={handleAdd} className="mb-12 bg-cyber-navy border border-cyber-sapphire/20 p-8 rounded-2xl animate-fade-in text-right">
            <h3 className="text-xl font-black text-white mb-6 border-b border-cyber-powder/10 pb-4">
              إنشاء محتوى جديد
            </h3>
            <div className="grid md:grid-cols-2 gap-6 mb-6">
              <div className="space-y-2">
                ...
                <input 
                  ...
                  className="w-full bg-cyber-navy border border-cyber-powder/20 rounded-lg p-3 text-white focus:border-cyber-sapphire outline-none" 
                />
              </div>
              <div className="space-y-2">
            ...
            <div className="flex justify-end">
              <button type="submit" className="bg-cyber-sapphire px-10 py-3 rounded-lg font-black text-white">نشر المحتوى فوراً</button>
            </div>
          </form>
        )}
        ...
        <div className="grid gap-4">
          {(activeTab === 'articles' ? articles : stories).map((item: any) => (
            <div key={item.id} className="bg-cyber-ice/5 border border-cyber-powder/20 p-6 rounded-2xl flex items-center justify-between hover:border-cyber-powder/40 transition-all">
              <div className="flex gap-4">
                ...
                <button className="p-3 bg-cyber-ice/10 text-slate-400 rounded-lg border border-cyber-powder/20 hover:text-white transition-all">
              ...
              <div className="text-right">
                ...
                <div className="flex items-center justify-end gap-3 mt-1">
                  ...
                  <span className="text-[10px] px-2 py-0.5 bg-cyber-navy rounded text-cyber-sapphire font-bold uppercase">{item.category}</span>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="grid md:grid-cols-3 gap-6 mt-16">
          <div className="bg-cyber-ice/5 border border-cyber-powder/20 p-6 rounded-2xl text-right">
            <Database className="w-8 h-8 text-cyber-sapphire mb-4 ml-auto" />
          ...
          <div className="bg-cyber-ice/5 border border-cyber-powder/20 p-6 rounded-2xl text-right">
            <LayoutGrid className="w-8 h-8 text-cyber-sapphire mb-4 ml-auto" />
